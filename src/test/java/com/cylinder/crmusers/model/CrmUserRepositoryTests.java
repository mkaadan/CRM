...
public class CrmUserRepositoryTests extends RespositoryTests {
    ...
    public void testFindByAccountId() {
        ...
        assertEquals(user == null, false);
        assertEquals(user.getLastName() == "McGee", true);
        user = userRepository.findByAccountId(new Long("3"));
        assertEquals(user == null, true);
    }

    @Test
    public void testExistsByAccountId() {
        assertEquals(userRepository.existsByAccountId(new Long("1")), true);
        assertEquals(userRepository.existsByAccountId(new Long("2")), false);
    }

    @Test
    public void testExistsByAccountIdAndEmail() {
        assertEquals(userRepository.existsByAccountIdAndEmail(new Long("4"), "testball@gmail.com"), true);
